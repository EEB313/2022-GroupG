...
```{r}
#create linear model 1 (x = temperature, y = flowering day)
...
```{r}
#predicted values of first flowering date by temperature based on linear model
flower_temp_data %>% 
  mutate(flowering_date_pred = predict(flower_temp_lm)) %>% #include predicted values to dataset
  ggplot(aes(x = mean_annual_temp, y = day)) +
  geom_line(aes(y = flowering_date_pred)) +
  theme_classic() +
...
flower_temp_lm2_linearity <- plot(resid(flower_temp_lm2))
```
# looks random, so we have not violated the assumption of linearity (https://ademos.people.uic.edu/Chapter18.html)

```{r}
#assumption 2 - homogeneity of variance

#extract the residuals and places them in a new column in our original data table
flower_temp_data$flower_temp_lm2.Res <- residuals(flower_temp_lm2) 

#create a new column with the absolute value of the residuals
flower_temp_data$flower_temp_lm2.Res <- abs(flower_temp_data$flower_temp_lm2.Res)

#square the absolute values of the residuals to provide the more robust estimate
flower_temp_data$flower_temp_lm2.Res2 <- flower_temp_data$flower_temp_lm2.Res^2

#ANOVA of the squared residuals
Levene.flower_temp_lm_2 <- lm(flower_temp_lm2.Res2 ~ species_name, data = flower_temp_data)

anova(Levene.flower_temp_lm_2) #displays the results
```
#Since the p-value is greater than 0.05, the assumption of homogeneity of variance is met.

```{r}
#assumption 3 - normal distribution
qqmath(flower_temp_lm2)
```
#Overall, the line looks straight, which suggests that the assumption of normality is not violated.

```{r}
# create mixed effect model (temperature vs flowering date + station (random effect))

flower_temp_lm3 <- lmer(day ~ mean_annual_temp + (1|species_name) + (1|island/station_name), data = flower_temp_data, REML = FALSE)       
...
```
```{r}
# create graph of temperature (x) vs. flowering day (y) by stations using actual data
