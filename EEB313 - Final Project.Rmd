...
temp_humidity <- ggscatter(avg_weather, x = "mean_avg_temp", y = "mean_avg_relative_humidity", add = "reg.line", conf.int = TRUE, cor.coef = TRUE,
          cor.method = "kendall",
          xlab = "Average Temperature", ylab = "Average Relative Humidity (%)")
temp_humidity
```
```{r}
...
lm3.b.fig <- flower_temp_data %>% 
  ...
  theme_classic() +
  facet_wrap(~island) +
  labs(x = "Mean Annual Temperature (Degrees Celsius)", y = "First Flowering Date (Predicted Data)", colour = "Species" ) +
